...
The current implementation also includes a cohesive deadline-aware media pipeline (`MediaSender`/`MediaReceiver`) that fragments access units, adds per-frame XOR FEC, retains audio/keyframes for retransmission, drops expired frames, and drives NACK feedback — demonstrated end to end by `fluxcast-cli pipeline-demo`. It further provides a signed forward-secret session handshake (Ed25519 + ephemeral X25519 + HKDF), ChaCha20-Poly1305 packet protection with replay rejection, RFC 5389 STUN server-reflexive candidate discovery, multi-subscriber relay leases/metrics, and CLI H.264 Annex-B / Ogg Opus send-and-recover paths. A deterministic loss/reordering/expiry simulator (`fluxcast-cli simulate`) demonstrates FEC recovery without a live network, and `spec/test-vectors.json` pins canonical FCDP framing that the Rust, Python, and Node.js SDKs are all checked against. These capabilities have local and public-STUN runtime checks, but the project is still pre-alpha: ICE nomination/TURN allocation, connection migration, browser interoperability, stable SDKs, production relay control-plane persistence, long-duration impairment testing, and an independent security review are unfinished. Do not use it for production or confidential media.
