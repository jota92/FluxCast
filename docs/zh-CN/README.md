...
首要产品目标是：**即使在不稳定的移动网络上，也能持续提供超低延迟媒体传输**。
网络切换、丢包、抖动和带宽变化下的连接连续性与延迟，必须通过可重复的测试证明，
才可作为发布条件；目前并不声称已经达到该目标。具体标准请参见
[ROADMAP.md](../../ROADMAP.md) 与 [VALIDATION.md](../../VALIDATION.md)。
