...
FCDP のパケット形式と公開 API は、安定版まで変更される場合があります。

最優先の製品目標は、**不安定なモバイル回線でも使い続けられる超低遅延配信**です。
回線切替、損失、ジッター、帯域変動に対する接続継続性と遅延を、再現可能な試験で
示すことをリリース条件にします。現時点では、この目標を達成したという主張では
ありません。詳しい基準は [ROADMAP.md](../../ROADMAP.md) と
